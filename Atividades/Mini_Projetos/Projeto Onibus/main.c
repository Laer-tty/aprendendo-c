...
void monta_tela() {
        ...
        printf("\n8 - Excluir terminais iguais em sequencia");
        printf("\n9 - Sair");
...
// =========================================== EXCLUIR TERMINAIS IGUAIS EM SEQUÊNCIA ============================================
}

void excluir_terminal_sequencia(tipo_lista l) {
    if(tamanho(l) == 0) {
    printf("\nLista vazia!\n");
    }

    int i = 1;elemento e1; elemento e2;
    while (i <= tamanho(l)) {
        obter_elemento(l, i, &e1);
        obter_elemento(l,i+1,&e2);

        if (e1.terminal == e2.terminal && e1.placa == e2.placa) {
            excluir_elemento(&l,i + 1);
   ...
   do {
   	...
   	switch (opcao) {
      	...
      	case '7':
            ...
            break;
        case '8':
            excluir_terminal_sequencia(lista);
            break;
        case '9':
            exit(1);
            break;
